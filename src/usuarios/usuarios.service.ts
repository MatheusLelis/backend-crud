...
export class UsuariosService {
  ...
  constructor(
    ...
    @InjectRepository(FaixaEtaria)
    private FaixaEtariaRP: Repository<FaixaEtaria>,
  ) { }
  ...
  findAllFaixasEtarias() {
    return this.FaixaEtariaRP.find()
  }
  ...
  updateStatus(idUsuario: number, idStatus: number) {
    console.log("updateStatus = ", idUsuario, idStatus )
    return this.usuariosRP.createQueryBuilder('usuarios')
      .update()
      .set({ idStatus: idStatus })
      .where(`id = :idUsuario`, {idUsuario: idUsuario})
      .execute().then(exclusao => {
        console.log("exclusao status = ", exclusao)
      })
  }
  ...
  async filtro(filtro: Filtro){
    ...
    const faixa = await this.FaixaEtariaRP.findOne(+filtro.idFaixaEtaria)
    ...
    const todosUsuariosFiltrados = todosUsuarios.filter( async usuario => {
      ...
      if(+filtro.idFaixaEtaria){
        const hoje = new Date()
        const hoje2 = new Date()
        const dataInicial = new Date(hoje.setFullYear(hoje.getFullYear() - faixa.limiteSup) )
        const dataFinal = new Date(hoje2.setFullYear(hoje2.getFullYear() - faixa.limiteInf) )
        const dataUsuario = new Date(usuario.dataNascimento)

        if(dataUsuario < dataInicial || dataUsuario > dataFinal) return
      }
